...
Multiplier::Multiplier(Pokemon pok1, Pokemon pok2, int pokemon_type) {
    pok1.multiplier = 1; // Default multiplier
    ...
    for (int i = 0; i < 3; ++i) {
        if (strengths[i] != 0 && pokemon_type == strengths[i]) {
            pok1.multiplier = 0.5;  // Less damage for strengths
    ...
    for (int i = 0; i < 2; ++i) {
        if (weaknesses[i] != 0 && pokemon_type == weaknesses[i]) {
            pok1.multiplier = 2;  // More damage for weaknesses
